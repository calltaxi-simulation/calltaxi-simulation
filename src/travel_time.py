"""
travel_time.py — 이동시간 산출 (실측 OD → 속도 테이블 → 직선거리 근사)

"동A → 동B 를 X시 Y요일에 가면 몇 분"을 돌려준다. 시뮬레이터가 공차이동·운행
구간마다 부르는 함수라 조회는 딕셔너리 1회로 끝나게 짰다.

3단 구조 — 임의의 동쌍을 빠짐없이 커버하기 위한 폴백 사슬:

  1순위 od      (출발동, 목적동, 시간대, 평일/주말) 실측 중앙값. 10건 이상만 신뢰.
  2순위 speed   실측이 없으면 중심점 직선거리 ÷ (거리대·시간대·요일 중앙속도)
  3순위 line    속도 테이블 셀도 비면 시간대·요일 평균속도로 같은 계산
  intra         출발동 == 목적동 (직선거리 0) — 실측 동일-동 중앙값, 없으면 5분
  default       중심점 좌표가 없는 동 — 해당 시간대 전체 운행시간 중앙값

⚠ 명세와의 차이 한 가지: 2순위에도 우회계수를 곱한다.
속도 테이블은 실측 `승차거리`(도로 주행거리) 기준으로 만든 값이라, 직선거리를
그대로 나누면 도로거리와 단위가 어긋나 우회계수만큼(약 36%) 과소추정된다.
2·3순위 모두 `직선거리 × DETOUR ÷ 속도` 로 통일했다. 두 순위의 차이는
거리대별 속도를 쓰느냐(2순위) 시간대 평균속도를 쓰느냐(3순위)다.

시간대 5구간 × 요일 2구분 = 10개 프로필. load.py 의 `PERIOD_LABELS`(4구간)와는
기준이 다르다 — 저쪽은 대기시간 집계용이고 여기는 피크 혼잡을 반영해야 해서
오전/오후 피크를 분리했다. 섞어 쓰지 말 것.

알려진 한계 (수치는 `python src/travel_time.py` 재현):

- **1순위 커버 79.9%.** OD쌍만으로 끊으면 89.9% 지만 시간대·요일까지 나누면 셀이
  얇아져 20%가 2순위로 내려간다(요일만 추가 86.7%, 시간대만 추가 83.1%).
  실측 대비 오차는 1순위 절대중앙 2.1분, 2순위 5.1분이라 감수할 만한 교환이다.
- **거리대는 5구간이다.** 처음엔 10km+ 가 한 칸이었는데 10km 와 30km 를 같이 담아
  25km 이상 2순위 건이 중앙 +12.5분 과대추정됐다. 20km 컷을 넣어 −0.7분으로 잡았다
  (위 DIST_CUTS 주석). 거리대를 더 손보려면 셀별 건수가 MIN_SPEED_TRIPS 이상인지
  검증 출력에서 확인할 것 — 얇은 셀은 표에서 빠져 3순위로 흘러간다.
- **동명만으로 조회한다.** `신사동`(강남·관악)처럼 구가 갈리는 동명은 좌표를
  평균한다. 1·2순위 키도 동명 기준이라 두 동의 실측이 한 셀에 섞인다.

원본 CSV 파싱이 13초쯤 걸려 산출 테이블은 cache/ 에 parquet 으로 떨군다.
원본 mtime·크기가 파일명에 들어가 있어 원본이 바뀌면 자동으로 다시 만든다.

    from travel_time import get_travel_time
    get_travel_time("역삼1동", "논현1동", hour=8, is_weekend=False)   # → 분

검증 출력(커버율·샘플 조회·이상값)은 `python src/travel_time.py`.
"""
...
MIN_SPEED_TRIPS = 30    # 속도 셀 신뢰 최소 건수 — 셀이 40개뿐이라 넉넉히 잡는다
