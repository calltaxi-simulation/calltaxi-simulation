...
| 행정동_중심점.csv | 동 중심 좌표 |
| 서울시_장애인_통계_2025.csv | 동별 등록 장애인(이용률 분모). 이용률은 지도 색칠에서 빠지고 패널 표시용이다 — 낮은 이용률이 소외인지 낮은 수요인지 가릴 수 없어 방향을 정할 수 없다) |
| HangJeongDong_ver20230701.geojson | 동 경계 |
| 동별_거점용량_접근성.csv | 과소공급 동(3km내 10대 이하) |
| sim_pool_v4.csv | **거점 후보 풀 639곳 · 55,798면** — 만들어진 과정은 원본 파이프라인 명세(출처는 docs/external_sources.md). 옥외 348곳만 후보로 쓰고(가정 A-15), 그중 동에 배정된 244곳이 시뮬에 들어간다(candidates.py) |
...
| 판정 대상 | 연간 실측 | 허용 |
|---|---|---|
| 픽업 평균 / 중앙 / p90 | 18.8 / 17.8 / 29.4분 | 0.5 / 0.5 / 1.0 |
| 취소율 | 15.44% | 0.002 |
| **픽업 지니** (`pickup_gini`) | 0.0704 | 0.005 |
...
**시뮬 차량 대수는 723이 아니라 일별 가동 중앙 561대**(평일 574 / 주말 374)다.
723은 연간 누적 고유 차량번호라 그대로 넣으면 공급이 29% 과대해진다
(`metrics.daily_active_vehicles()`).
