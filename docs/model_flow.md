...
## 후보 비교의 잡음 바닥 (2026.08.09) — **전체 실행 전에 읽을 것**

배치안 5개를 대표 기간으로 시험 실행하며 잰 값이다. **경로는 정상이다** — 출력 컬럼
구조가 before 와 같고, 픽업이 5개 전부 줄었으며(−0.055 ~ −0.086분), 승차 건수가
늘었다(+100~150건). A-10 배분도 확인했다: 691 → 701 이면 평일 574 → 582대이고
신규 거점이 8대를 받으며 **기존 44곳 배속은 전부 그대로**다.

**그런데 후보 간 차이를 가릴 수 없다.** 같은 후보를 시드 3개로 돌려 재본 값:
...
**차이(0.013)가 잡음(0.039)의 1/3이다.** 3회 반복으로 후보 하나의 효과가 0 이
아니라는 것은 겨우 잡히지만(t≈2.5~3.1), 두 후보 중 어느 쪽이 나은지는 못 가린다.
0.013분을 표준오차 0.0065 로 잡으려면 **후보당 36회 반복**이 필요하고, 244개 ×
36회 × 45초 = 110시간이라 불가능하다.

**공통난수로 줄일 수 있는 몫은 이미 줄였다.** 콜 단위 난수(인내심·취소)는 call_id
로, 조 편성은 **차량 슬롯 ID** 로 고정했다 — 슬롯 ID 로 바꾸기 전에는 증차로
가동 대수가 574 → 582 가 되는 순간 전 차량의 조 편성이 통째로 달라져 잡음이 더
컸다(동별 픽업 표준편차 0.465분). 남은 변동은 **계 자체의 카오스**다 — 차 8대가
늘면 배차 순서가 바뀌고, 그 뒤로 모든 차량의 궤적이 갈린다. 난수를 고정해도 이건
남는다.

**그래서 전체 평균으로 순위를 매기면 안 된다.** 효과가 244개 동에 희석되기 때문이다.
같은 실행에서 **동별로는 최대 −1.2 ~ −2.0분**까지 개선된다 — 거점 인근에서는 신호가
20~30배 크다. 평가 지표를 **거점 인근 동의 픽업 개선**으로 좁히면 잡음 대비 신호가
살아난다. 전체 실행 전에 이 지표를 확정해야 한다(미결).
